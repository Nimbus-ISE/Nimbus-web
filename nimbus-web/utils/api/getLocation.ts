...
const getLocation = async (loc_id: string) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            string_agg(DISTINCT B.tag_name, ', ') as full_tag_list,
            string_agg(DISTINCT I.url, ', ') as url
            FROM location_data L 
            JOIN review R ON L.loc_id = R.loc_id 
            JOIN belong_to B ON L.loc_id = B.loc_id
            JOIN image I ON L.loc_id = I.loc_id 
            WHERE L.loc_id = ${loc_id}
            GROUP BY L.loc_id, R.review_text, R.author, R.review_date, R.rating, R.url
            `;
